...
// The goal of this analyzer is to take in any number of files, and combine them all into a single file based on the timestamp
...
public class InterpolaterProAnalyzer extends Analyzer {
    ...
    public void analyze() throws IOException, CsvValidationException {
        ...
        // Then we can create the header using the first file along with the extra column from the other files
        ...
        // Now, using the readers, we need to find which column is the timestamp column and which is the data column in each file
        // We can do this by looking at the first row of each file and using the getAnalysisColumnIndex function
        ...
        // Ideally, our final file should have a set of timestamps that is the union of all the timestamps in the input files
        // For each corresponding data point, if we can take the two closest points and interpolate between them, we can get the data point for that timestamp
        ...
        PriorityQueue<TimestampData> queue = new PriorityQueue<TimestampData>((a, b) -> Double.compare(a.timestamp, b.timestamp));
        ...
        // Now, we need to keep reading through every other file except the maximum until we reach the maximum timestamp
        for (int i = 0; i < readers.size(); i++) {
        ...
        while(!queue.isEmpty()) {
            // Each loop, we will take the top element from the queue, then create a datapoint for all the files and write it
            ...
            if (queueData.timestamp != queuePreviousTimestamp) {
                ...
                for (int i = 0; i < readers.size(); i++) {
                    ...
                    if (currentTimestamp[i] == queueData.timestamp) {
                        ...
                        double interpolatedData = interpolate(previousTimestamp[i], previousData[i], currentTimestamp[i], currentData[i], queueData.timestamp);
            ...
            currentTimestamp[queueData.fileIndex] = Double.parseDouble(line[timestampIndices[queueData.fileIndex]]);
    ...
    public static void main(String[] args) {
        String[] inputFiles = {"X:/Code/Projects/Baja/Better-Data-Viewer/API/upload-dir/151408/GPS LATITUDE.csv", 
        ...
        InterpolaterProAnalyzer analyzer = new InterpolaterProAnalyzer(inputFiles, inputColumns, outputFiles);
        try {
            analyzer.analyze();
        } catch (Exception e) {
