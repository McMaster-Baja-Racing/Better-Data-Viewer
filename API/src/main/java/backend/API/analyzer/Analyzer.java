...
public abstract class Analyzer {
  ...
  // Factory method allows creation of different types of analyzers without having to change the
  // code that calls it
    ...
    switch (type) {
      case "accelCurve":
        if (outputFiles.length == 10) {
          // Concept here is when no output files are provided to format automatically, the last one
          ...
          outputFiles[2] =
              inputFiles[0].substring(0, inputFiles[0].length() - 4)
                  + "_inter_"
  ...
  public static void main(String[] args) {
    ...
    // String[] inputFiles =
    // {"X:\\Code\\Projects\\Baja\\Better-Data-Viewer\\data\\live_F_RPM_PRIM.csv"};
    ...
    // String[] inputFiles2 =
    // {"X:\\Code\\Projects\\Baja\\Better-Data-Viewer\\data\\live_F_RPM_PRIM.csv",
    // "X:\\Code\\Projects\\Baja\\Better-Data-Viewer\\data\\live_F_RPM_SEC.csv"};
    // String[] outputFiles2 = {"prim_average.csv", "sec_average.csv", "interpolate.csv",
    // "accel_curve.csv"};
