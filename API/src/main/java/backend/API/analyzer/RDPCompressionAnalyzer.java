...
public class RDPCompressionAnalyzer extends Analyzer {
  ...
  public void analyze() {
    ...
    final Reader r = new CSVReader(inputFiles[0]);
    final Writer w = new CSVWriter(outputFiles[0]);
  ...
  public List<List<String>> ramerDouglasPeucker(List<List<String>> data, double epsilon) {
    ...
    if (maxDistance > epsilon) {
      ...
      List<List<String>> secondLine =
          ramerDouglasPeucker(data.subList(maxIndex, data.size()), epsilon);
  ...
  private double perpendicularDistance(
      List<String> lineStart, List<String> lineEnd, List<String> point) {
