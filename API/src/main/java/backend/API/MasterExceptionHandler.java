...
public class MasterExceptionHandler 
  extends ResponseEntityExceptionHandler {
    ...
    protected ResponseEntity<Object> handleBadCSV(
      RuntimeException ex, WebRequest request) {
        String bodyOfResponse = "Error: " + ex.getClass().getCanonicalName() + " → " + ex.getMessage() + ".\nMessage: This is most likely caused by a faulty CSV file. Make sure to check for any empty cells, trailing whitespaces or \\n characters.";
        HttpHeaders responseHeaders = new HttpHeaders();
        responseHeaders.add(HttpHeaders.ACCESS_CONTROL_ALLOW_ORIGIN, "*");
		    responseHeaders.add(HttpHeaders.ACCESS_CONTROL_ALLOW_CREDENTIALS, "true");
        return handleExceptionInternal(ex, bodyOfResponse, 
          responseHeaders, HttpStatus.CONFLICT, request);
    }

    // Method to handle issues with analyzer options.
    @ExceptionHandler(value 
      = { NumberFormatException.class, NotStrictlyPositiveException.class, IndexOutOfBoundsException.class })
    protected ResponseEntity<Object> handleBadAnalyzerOption(
      RuntimeException ex, WebRequest request) {
        String bodyOfResponse = "Error: " + ex.getClass().getCanonicalName() + " → " + ex.getMessage() + ".\nMessage: Make sure to check your analyzer options before submitting your request.";
        HttpHeaders responseHeaders = new HttpHeaders();
        responseHeaders.add(HttpHeaders.ACCESS_CONTROL_ALLOW_ORIGIN, "*");
		    responseHeaders.add(HttpHeaders.ACCESS_CONTROL_ALLOW_CREDENTIALS, "true");
        return handleExceptionInternal(ex, bodyOfResponse, 
