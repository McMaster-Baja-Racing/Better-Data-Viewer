...
const Views = ({ viewInformation, setModal, setButtonID, numViews, videoTimestamp, setVideoTimestamp }) => {
    return (
            ...
            {Array.from({ length: numViews }).map((_, index) => {
                ...
                return (
                    <div key={index} className="singleView" style={{ gridColumn: viewStyles[numViews].gridColumn[index] }}>
                        <Component {...props} videoTimestamp={videoTimestamp} setVideoTimestamp={setVideoTimestamp} />
