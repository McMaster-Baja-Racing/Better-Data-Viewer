  ...
  const updatePage = (pageVar) => {
    ...
    if (displayPage === 2) {
      return (<FileStorage files={files} selectedFiles={selectedFiles} setSelectedFiles={setSelectedFiles} setDimensions={setDimensions} setColumns={setColumns} setDisplayPage={setDisplayPage}/>)
        ...
        return (<AnalyzersAndSeries setSuccessMessage={setSuccessMessage} success={success} dimensions={dimensions} columns={columns} setDisplayPage={setDisplayPage} setShowModal={setShowModal} handleSubmit={handleSubmit}/>)
    } else{
