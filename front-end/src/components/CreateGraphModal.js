...
export const CreateGraphModal = ({ setShowModal, setChartInformation,success, setSuccessMessage }) => {
  ...
  const updatePage = (pageVar) => {
    ...
    if (displayPage === 2) {
      return (<FileStorage files={files} selectedFiles={selectedFiles} setSelectedFiles={setSelectedFiles} setDimensions={setDimensions} setColumns={setColumns} setDisplayPage={setDisplayPage}/>)
        ...
        return (<AnalyzersAndSeries dimensions={dimensions} columns={columns} setDisplayPage={setDisplayPage} setShowModal={setShowModal} handleSubmit={handleSubmit} setSuccessMessage={setSuccessMessage} success={success}/>)
    } else{
